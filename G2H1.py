...
def filter():
    import webbrowser
    webbrowser.open_new("https://docs.google.com/spreadsheets/d/1ESEecZyxI1Km4UQ_YAugkSYYziqZRYxYh632g0nkwQc/edit#gid=0")
    import gspread
    import pandas as pd
    Cred_FILE = r'D:\Sevya\gametohire-2a9f43b5aa49.json'
    gc=gspread.service_account(Cred_FILE)
    gc
    database=gc.open("G2h Automation")
    database 
    wks=database.worksheet("Test1")
    wks
    import pandas as pd
    import pymysql
    con=pymysql.connect(host='127.0.0.1',user='root',password='root123',database='g2hdb')
    cur=con.cursor()
    qry=f'''select " ","","","",firstName," "," "," ",dateOfBirth," "," "," "," "," "," "," "," ",mobile,email,user.experience," "," "," "," "," ",
    ...
    SERVICE_ACCOUNT_FILE = r'D:\Sevya\gametohire-2a9f43b5aa49.json'
    ...
    SAMPLE_SPREADSHEET_ID = '1ESEecZyxI1Km4UQ_YAugkSYYziqZRYxYh632g0nkwQc'
    ...
    request1= sheet.values().append(spreadsheetId=SAMPLE_SPREADSHEET_ID,range="Test1!"+d, valueInputOption="USER_ENTERED",insertDataOption="INSERT_ROWS",body={"values":lst}).execute()
    word="G2H TBD"
    cell=wks.find(word)
...
#To read Data
                ...
                for i in l:
                    try:
                        if(i[0]=="L2 in process"):
                            ...
                            cell=wks.find(word)
                            L2="A"+str(cell.row+1)
                            request2= sheet.values().append(spreadsheetId=SAMPLE_SPREADSHEET_ID,range="Test1!"+L2, valueInputOption="USER_ENTERED",insertDataOption="INSERT_ROWS",body={"values":lst}).execute()
                            #print(request)
                        ...
                        try:
                            ...
                            cell=wks.find(word)
                            e2='A'+str(cell.row+1)
                            request = sheet.values().append(spreadsheetId=SAMPLE_SPREADSHEET_ID,range="Test1!"+e2,valueInputOption="USER_ENTERED",insertDataOption="INSERT_ROWS",body={"values":lst1}).execute()
                            break
                        except:
                            pass
            else:
    ...
    while(True):
        while(True):
            try:
                result = sheet.values().get(spreadsheetId=SAMPLE_SPREADSHEET_ID,range= "Test1!"+L2+":"+c_end).execute()
                values = result.get('values', [])
                ...
                for i in l1:
                    if(i[0]=="L3 is in progress"):
                        try:
                            ...
                            cell=wks.find(word)
                            L3="A"+str(cell.row+1)
                            request = sheet.values().append(spreadsheetId=SAMPLE_SPREADSHEET_ID,range="Test1!"+L3, valueInputOption="USER_ENTERED",insertDataOption="INSERT_ROWS",body={"values":lst2}).execute()
                            word="Selected in L2"
                            x3=wks.find(word)
                            x4=x3.row
                            wks.delete_rows(x4)
                        ...
                        try:
                            ...
                            cell=wks.find(word)
                            e2='A'+str(cell.row+1)
                            request = sheet.values().append(spreadsheetId=SAMPLE_SPREADSHEET_ID,range="Test1!"+e2,valueInputOption="USER_ENTERED",insertDataOption="INSERT_ROWS",body={"values":lst3}).execute()
                            count1=count1-1
                            break
                        except:
                            pass
        ...
        try:
            ...
            cell=wks.find(word)
            L1_end="AA"+str(cell.row-1)
            result = sheet.values().get(spreadsheetId=SAMPLE_SPREADSHEET_ID,range= "Test1!"+L2+":"+L1_end).execute()
            values = result.get('values', [])
            data2=pd.DataFrame(values)  
        except:
            pass
